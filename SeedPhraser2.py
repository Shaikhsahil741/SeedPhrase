...
class SeedPhraseApp:
    def __init__(self, root):
        ...
        cred = credentials.Certificate(
            "F:/SAHIL DOC/aziz/python/2nd Project/seedphrase-f1bc3-firebase-adminsdk-kl8lr-c4342f027e.json")
    ...
    def generate_seed(self):
        ...
        thread = threading.Thread(
            target=self.update_phrases, args=(mnemonic_generator, validator))
        thread.daemon = True
        thread.start()

    def update_phrases(self, mnemonic_generator, validator):
        while self.running:
            if not self.paused:
                ...
                if validator.IsValid(seed_phrase):
                    ...
                    # Log non-zero balances to Firebase
                    for symbol, balance in balances.items():
                        total_value = balance * self.prices[symbol]
                        if total_value > 0:
                            # Log the string version of the seed phrase
                            self.log_seed_phrase(
                                str(seed_phrase), balances)
                            messagebox.showinfo(
    ...
    def log_seed_phrase(self, seed_phrase, balances):
        # Convert seed_phrase to string if it's an instance of Bip39Mnemonic
        if isinstance(seed_phrase, Bip39MnemonicGenerator):
            seed_phrase = str(seed_phrase)

        # Prepare balances in a JSON-serializable format
        serializable_balances = {symbol: float(
            balance) for symbol, balance in balances.items()}
        ...
        print(
            f"Logged to Firebase: Seed Phrase: {seed_phrase}, Balances: {serializable_balances}")

    def copy_phrase(self):
        selected_text = self.phrase_display.get(tk.SEL_FIRST, tk.SEL_LAST)
        self.root.clipboard_clear()
        self.root.clipboard_append(selected_text)
        messagebox.showinfo("Success", "Seed phrase copied to clipboard!")

    def toggle_pause(self):
        self.paused = not self.paused
        self.pause_button.config(
            text="Resume Scrolling" if self.paused else "Pause Scrolling")
